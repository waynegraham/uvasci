		...
		if($post->post_parent) { // page is a child
			...
			if($parent->post_parent) { // if page is a grandchild
				...
				if($grandparent->post_parent) { // if page is a greatgrandchild
					...
					$tertiarynav = wp_list_pages('sort_column=menu_order&title_li=&child_of='.$grandparent->ID.'&echo=0');
				}
					...
					$tertiarynav = wp_list_pages('sort_column=menu_order&title_li=&child_of='.$parent->ID.'&echo=0');
				}
			}
				...
				$tertiarynav = wp_list_pages('sort_column=menu_order&title_li=&child_of='.$post->ID.'&echo=0');
			}
		}
...
<?php	 	
	if ($secondarynav) { ?>
		<ul id="subnav" class="clearfloat">
			<?php	 
			<li class="page_item page-item-10"><a href="/current-institute/steering-committee/" title="Steering Committee">Steering Committee</a></li>
		</ul>
	<?php	
	}
?>
